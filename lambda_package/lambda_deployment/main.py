...
@app.post("/api/python/generate-payment-link")
...
def lambda_handler(event, context):
    try:
        ...
        # Process the request using your existing FastAPI endpoint logic
        result = agent_executor.invoke({
            "input": f"""Create a payment link for this request: {body.get('message', '')}
            Follow these steps exactly:
            1. Create a product based on the request
            2. Create a price for that product
            3. Create and return a payment link
